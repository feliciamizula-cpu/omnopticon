...
**NOTE: You must recreate the Dockerfile.service first if missing:**
...
COPY src/Argus.Web/ "$PROJECT_PATH"
...
EXPOSE 8080
ENTRYPOINT ["sh", "-c", "dotnet ${ASSEMBLY_NAME}.dll"]
```
...
docker build -f deploy/Dockerfile.service \
  ...
  --build-arg PROJECT_PATH=src/Argus.Web/Argus.Web.csproj \
  --build-arg ASSEMBLY_NAME=Argus.Web \
  -t argus-web:latest .
...
2. **Manual Docker**:
   - [ ] `cd deploy`
   - [ ] Recreate Dockerfile.service if missing
   - [ ] `docker build -f deploy/Dockerfile.service ...`
   - [ ] `docker push` (if using remote registry)
   - [ ] `gcloud run deploy argus-web-staging ...`
